...
class arrayAccessSet extends  \Orm\model{
	protected static $_properties = array("name" ,
	"age" ,
	"weight"  ,
	"location"  
	); 	
...
class Test_Arr extends TestCase
{
	 ...
	 *  
	 */
	public function test_get_with_array_access()
	{
		$expected = "ohm";
		$model = new arrayAccessSet();
		$model->name="ohm";
		$model->age="22";
		$model->weight="222";
		$output = Arr::get($model, "name", "Unknown Name");
		$this->assertEquals($expected, $output);
	}
}
