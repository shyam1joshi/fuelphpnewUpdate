...
class Controller_Forms extends \Controller_Base{
    ...
    private $postDataReceivedFields = array(    'json_data' => 'json_data', 
                                                ...
                                                'logo_id' => 'logo', 
                                                'flowType' => 'flowType', 
                                                'setCrudPermissions' => 'setCrudPermissions', 
                                                'flowSystem' => 'flowSystem', 
                                                'formtype' => 'formtype', 
                                                'formIndexTitle' => 'formIndexTitle', 
                                                ...
                                                'addAccessibilityMenu' => 'addAccessibilityMenu', 
                                                ...
                                                'mutli_url_validity' => 'mutli_url_validity', 
                                                'formId' => 'formId', 
                                               ...
                                               'dont_store_data' => 'dont_store_data' 
//                                                'menuList' => 'menuList',  
                                    );
    ...
    private $defaultProperties = array('id','name','agent_id', 'mobile', 'flow','key_to', 'confirm', 'deleted', 'submitted_date', 'submittedupdate_date', 'created_at', 'updated_at', 'create_uid', 'update_uid','link_expire_date');
        
    private $fieldtypes = array('radio-group','time', 'autocomplete' , 'checkbox-group' ,'phonenumber', 'select','canvas', 'date', 'file', 'text', 'textarea','bankdetails','editabletext');
    ...
    private $textFields = array('textarea', 'checkbox-group','checkbox','radio','editabletext');
...
//    private $mainModulePath = APPPATH.'modules'.DS.'dynamicformssystem'.DS.'modules';                
    ...
    public function action_edit($id = null) {
        ...
        $data['menulist'] = $this->getMenuList($id);
    ...
    public function getMenuList($id){
        ...
        if(is_object($module)){
        $menu = Model_Menu::query()->where('modulename', $module->name)->get_one();
        ...
        if(is_object($menu)){
            ...
            $menulist = Model_Menu::query()
                    ...
                    ->where('module_id', '!=', $id)
                    ->where('module_id', '!=', $menu->module_linked_to)
                    ->get_array();
        }
        }
...
//        $sysobj =  \Model_Systemconfig::query()->where("name",$form_name)->get_one(); 
    ...
    public function before() {
        ...
        $postloginMethods = \Input::post('loginMethods',null); 

        foreach ($escapeArray as $key => $method){

            if(is_array($postloginMethods) && count($postloginMethods) > 0 && in_array($method, $postloginMethods) ){
                $systemsconfig[$method] = '0';
   
            }else{
                $systemsconfig[$method] = '1';
            }
        }
...
//        $systemsconfig['modified'] = 1;
    ...
    public function storeLatestRecordEx(){
        ...
        if(isset($this->dont_store_data) && $this->dont_store_data == 1){
            ...
            $object = $model::query()->order_by('id', 'desc')->get_one();
            ...
            if(is_object($object)){
                ...
                if(is_array($has_many_relations) && count($has_many_relations) > 0){
                    ...
                    foreach ($has_many_relations as $key=>$relation){
                        ...
                        $related_data[$key] = $object->$key;

                        $reltable = $relobj::table(); 
                        
                        \DB::delete($reltable)->execute();  
                ...
                if(count($related_data) > 0){
                        ...
                        foreach($related_data as $key => $reldat){
                            $objectNew->$key = $reldat;
                        }
    ...
    public function storeMenu($title = null, $formid = null) {
        
        if(!is_null($title)){
            $menuList = \Input::post('lines');
            ...
            if(!is_object($obj)){
                ...
                if(empty($obj->module_linked_to)){
                    $obj->module_linked_to = $formid;
                }

                $obj->module_id = $formid;

                $obj->menu_name = $this->menu_title;

                $obj->translation = $this->menu_title;  
                
                $obj->save() ;   
                                        
             $menuobjs = Model_Menu::query()->where('module_id','!=',$formid)
                     ->where('module_linked_to',$formid)->get();
                    
            
            if(is_array($menuobjs) && count($menuobjs) > 0){
                foreach($menuobjs as $ob){
                    $ob->delete();
                }
            }
            ...
            if(is_array($menuList) && count($menuList) > 0){
                ...
                foreach($menuList as $line){
                    ...
                    $menuobj = Model_Menu::query()->where('modulename',$line['name'])->get_one();
                    
                    if(!is_object($menuobj)){
                        $menuobj = new Model_Menu();
                    }
                    
                    $form = Model_Forms::query()->where('name',$line['name'])->get_one();
                    
                    $menuobj->module_linked_to = $formid;
                    
                    $menuobj->modulename = $line['name'];
                    
                    $menuobj->module_id = is_object($form)?$form->id:"";
                    
                    $menuobj->menu_name = $line['translation'];
                    
                    $menuobj->translation = $line['translation'];
...
//        $json_data = htmlspecialchars($this->json_data);
        ...
        if( !is_null($data)){    
            ...
            $sysobj->mutli_url_validity = $this->mutli_url_validity;
            ...
            $sysobj->iconcolor = $this->iconcolor;
            $sysobj->bordercolor = $this->bordercolor;
            $sysobj->email = $this->email;
            $sysobj->shorten_url = $this->shorten_url;
            $sysobj->enableAuthentication = $this->login;
            
            $sysobj->enableapi = \Input::post('enableapi');
            $sysobj->apikey = \Input::post('apikey');

            $sysobj->save();
            
            
            $this->storeMenu($title,$sysobj->id);
    ...
    public function replaceDashInName() {
        ...
        if(is_array($fields) && count($fields) > 0 )
            foreach ($fields as $fld){
                ...
                if(key_exists('copiedfrom', $fld) && !empty($fld['copiedfrom']))
                    $fld['copiedfrom'] = str_replace('-', '_', $fld['copiedfrom']);
...
//replaceDashInName
    ...
    public function sortCopyList() {
        ...
        foreach($fields as $val){
                ...
                if(key_exists('copiedfrom', $val) 
                        && !empty($val['copiedfrom'])
                        &&  $val['copiedfrom'] != '--select--'
                ){
                
                    $copiedfrom[] = array( 'name'=> $val['name'], 'copiedfrom' => $val['copiedfrom']);
                } 
        }  
        ...
        foreach($fields as $key => $val){
            ...
            if(key_exists('name', $val)){
                ...
                foreach($copiedfrom as $val1){
                
                    if($val['name'] == $val1['copiedfrom']){
                        $className = $fields[$key]['className'];
                        $fields[$key]['className'] = $className.' updateSimilarDataField ' ;
                ...
                foreach ($fields as $field){
                    ...
                    if( key_exists('type', $field) && $field['type'] ==  'section'){
                        ...
                        if(key_exists('section', $field) && $field['section'] == $section['name']){
                            ...
                            $field['className'] = 'form-control form-fields span6';
...
class Controller_{$class_name} extends \dynamicformssystem\Controller_Basesystemlogin
{
    {$contents}
...
//      class Controller_{$class_name} extends \dynamicformssystem\Controller_Base
    ...
    public function getControllerProperties($class_name = null) {
        ...
        $controllerProperties = <<<CONTENTS
            ...
            public \$uploadFolder =  '{$this->filename}';
...
PROP;
                         $this->noListView = 0;
                            ...
                            if($field['type'] == 'phonenumber'  ){
                             ...
                             //   $textname =  str_replace("editabletext","checkbox_group",$namex);
                                $phonecode =  str_replace("phonenumber","phonenumber_code",$namex);
                                ...
                                $this->noListView = 1;
                                  $prop .= <<<PROP
                                '$namex' => {$this->formatProperty($field)}
                                '$phonecode' => {$this->formatProperty($field,'phonecode')} 
...
PROPERTIES;
    ...
    public function mapField($field = array(), $type = null ) {
        
        $tempfield = '';
        $tempoptions = '';
        $propertyattr = '';
         $savelatest = $this->getKeyValue($field,'savelatest',false); 
         $fieldtypeedit = $this->getKeyValue($field,'type',false); 
...
TEMPFIELD;
                ...
                if(key_exists('type', $field) && $field['type'] == 'phonenumber'){
...
TEMPFIELD;
            ...
            if(key_exists('send_mail', $field) && $field['send_mail'] == 'yes'){
               $tempfield .= <<<TEMPFIELD
                                    'send_mail' =>  '1',

TEMPFIELD;
            }
            if(key_exists('send_link_sms', $field) && $field['send_link_sms'] == 'yes'){
                $tempfield .= <<<TEMPFIELD
                                     'send_link_sms' =>  '1',
 
TEMPFIELD;
             }
             if(key_exists('send_link_whatsapp', $field) && $field['send_link_whatsapp'] == 'yes'){
                $tempfield .= <<<TEMPFIELD
                                     'send_link_whatsapp' =>  '1',
 
TEMPFIELD;
             }
             if(key_exists('send_link_email', $field) && $field['send_link_email'] == 'yes'){
                $tempfield .= <<<TEMPFIELD
                                     'send_link_email' =>  '1',
 
TEMPFIELD;
             }
            if($fieldtypeedit == 'editabletext' && key_exists('savelatest', $field) && $field['savelatest'] == 'yes'){
               $tempfield .= <<<TEMPFIELD
                                    'savelatest' =>  '1',
...
PROPERTYATTR;
     ...
     * @return type
     */
    public function formatProperty($property = array(),$type = null ) {
                        
        $field = $this->getFieldType($property);       
        
        $propertyattr = $this->mapField($field,$type);
    ...
    public function getFieldType($field = null) {
        ...
        if(isset($type) && !is_null($type)){
            ...
            switch ($type){
                ...
                case 'editabletext'       : $type = 'longtext';  break;
            ...
            switch ($type){
                ...
                case \in_array($type,$this->varcharFields)   :   $field['dbtype'] = 'text';
                ...
                case   'longtext'              : $field['dbtype'] = 'longtext'; break;
...
<?php if(isset(\$addAccessibilityMenu) && \$addAccessibilityMenu == 1 ) { ?>
    <script>
  ...
  (function(page, header, body) { 
      ...
      window.access4u = { "sitekey": "64dde7ead1289e50847c6796", "userId": "6460bcd73b274aacc62b3c3a","feature": "ALL_FEATURES", "Menulang": "EN", "domains": { "js": ""} }; 
...
FOOTER;
    ...
    public function getMenu() {
       ...
       if($this->removesalessoftbar == 1){
            $hidebar = "<?php if(isset(\$removesalessoftbar) && \$removesalessoftbar == 1 ) { ?>style='display:none !important' <?php } ?>";
        }
...
// MENU;
        $menu .= <<<MENU
                ...
                <?php foreach( \$menuList as \$list ){ ?>
                 <li>
                        <a href="/<?php echo \$list['modulename'] ?>/<?php 
                        ...
                        echo \$modulename ?>/listindex"><?php  echo \$list['translation'] ?></a>
                    </li>
                <?php } ?>
                <?php } ?>
                <?php if(isset(\$loginmodule) ){ ?>
                    <li class="pull-left">
                        <a href="/<?php echo \$loginmodule ?>/base/logout"><?php echo \Lang::get('menu.logout'); ?></a>
                    </li>
                <?php } ?>
MENU;
        } 
...
<link href="//netdna.bootstrapcdn.com/font-awesome/3.2.1/css/font-awesome.css" rel="stylesheet">
   ...
   <link rel="stylesheet" href="/{$this->filename}/{$this->controller}/formcss">
                <link rel="stylesheet" href="/{$this->filename}/{$this->controller}/newformcss">
                <link rel="stylesheet" href="/{$this->filename}/{$this->controller}/formcsstwo">
                ...
                <link rel="stylesheet" href="/{$this->filename}/{$this->controller}/sectioncss">
                <link rel="stylesheet" href="/{$this->filename}/{$this->controller}/formcssresponsive">
                {$ltrcss}
...
CSS;
    ...
    public function getCreateForm() {
             ...
             $view  =  <<<CREATE
<form method='post' action="<?php // echo \$base;/create ?>" id="myFormId" path="/{$this->filename}/{$this->controller}"  js-url="/{$this->filename}/{$this->controller}/<?php if(isset(\$jsurl) && !empty(\$jsurl)) echo \$jsurl; else echo 'jsonOrdercomplete'; ?>" enctype='multipart/form-data' role="form" class="box-body">        
...
paragraph;
                            ...
                            case 'header' : $view .=<<<header
                                    {$field['subtype']} class='form-control span6'>
            {$field['label']} 
        </{$field['subtype']}>
header;
                                        break;
        ...
        if(is_array($fields)){
            ...
            foreach ($fields as $field){
                ...
                if(key_exists('type', $field) && $field['type'] == 'section' ){
                     ...
                     $steptoshown = $this->getKeyValue($field,'steptoshown',false);
                     $sectionview = $this->getKeyValue($field,'sectionView',false);
                     
                     $sectionparentclass = '';
                     $sectionchildclass = '';
                     $chevronclass = '';
                     
                     if($sectionview == '2'){ //for close
                         $sectionparentclass = ' collapsed-box ';
                         $sectionchildclass = ' collapse ';
...
//                                        $attributes['readonly'] = 'readonly';
                    ...
                    $iconcolor = $this->getKeyValue($field, 'iconcolor');
                    $bordercolor = $this->getKeyValue($field, 'bordercolor');
                    ...
                    if(!empty($bordercolor)){
                         $subsectionstyle .= ' border-bottom:3px solid '.$bordercolor.' !important;';
                     }
                    if(!empty($iconcolor)){
                         $iconstyle .= ' color: '.$iconcolor.' !important;';
                     }
			...
			$removeButtonCode  = "";
				    ...
				    $fielddataitems= $this->getElements($field['fields']);
					 ...
					 if(!(($this->viewType == 'edit' || $this->viewType == 'viewandsign') && $flow < 1)){
                        ...
                        $btnstyle ='display:block !important'; 
                        if(  $flow == 1 || $this->viewType == 'view'){ 
                             $btnstyle ='display:none;!important'; 
                        } 
                   $removeButtonCode = <<<fielddata
                                    <a class="btn pull-left removeFaultButton btn-modify-elmor" style="{$btnstyle}" id="Delete"><i style="display:block !important" class="icon-trash" title="Delete"></i></a>
                     ...
                     $fielddata .= <<<fielddata
                           <div class=" box-body form-group appendData" $hidediv>  
                            ...
                            <div class="box box-gray" $subsectionparentstyle>
                                <div class="box-header box-header-section  with-border bg-gray" $subsectionstyle data-widget="collapse" data-target="#lab" data-mini="true" data-theme="b">
								{$removeButtonCode}
                       <h4 class="box-title text-right"><span class="title">{$field['label']} {{$liname}}</span></h4>
                                    <a  class="chevronBtnUp pull-left" data-widget="collapse"><span class="icon-chevron-up icon-large" $iconstyle></span></a>                                    
                                </div>
...
SECTION;
                ...
                //    if(in_array($this->viewType, array('edit','viewandsign')) &&  $this->flowSystem >0 && $flow == 1 ){
                   if(  $this->flowSystem >0 && ( (in_array($this->viewType, array('edit','viewandsign')) && $flow == 1 )||  (in_array($this->viewType, array('create')) && $flow == 2 ))){
                        $btndisabled= 'disabled="disabled"';
                        $btnstyle ='style="display:none;"'; 
                   } 
                   $buttonCode .= <<<fielddata
                    <button  {$btndisabled} class="btn btn-primary"  {$btnstyle} clone-id="{$field['name']}_line" placeholder="" value="" autocomplete="off" clone-limit={$limitval} name="{$field['name']}" id="add{$field['name']}" >הוסף {$section_label}</button>
fielddata;
                     }
	...
	$fielddata .= " </div></div>";
                     ...
                     $removeButtonCode = "";
					   ...
					   if(!(in_array($this->viewType, array('edit','viewandsign')) &&  $this->flowSystem >0 && $flow == 1)){
                        ...
                        // } 
                   $removeButtonCode= <<<SECTION
                       <a class="btn pull-left removeFaultButton btn-modify-elmor" parent-button = "add{$field['name']}" style="{$btnstyle}" id="Delete"><i style="display:block !important" class="icon-trash" title="Delete"></i></a>
SECTION;
   }
                   ...
                   $fielddataclone = <<<SECTION
                            ...
                            <div class="box box-gray" $subsectionparentstyle >
                                <div class="box-header box-header-section  with-border bg-gray" $subsectionstyle data-widget="collapse" data-target="#lab" data-mini="true" data-theme="b">
								{$removeButtonCode}
                       <h4 class="box-title text-right" ><span class="title">{$field['label']}</span> </h4>
                                    <a  class="chevronBtnUp pull-left" data-widget="collapse"><span class="icon-chevron-up icon-large" $iconstyle></span></a>                                    
                                </div>
                                <div id="lab" class="box-body" data-inset="true" data-theme="e" style="text-align:right" data-content-theme="d" >
								$fieldDataMap
...
VIEWH;
                  ...
                  $field['label'] = htmlspecialchars_decode($field['label']);
                    $fielddata .= <<<SECTION
                            ...
                            <div class="box box-{$this->color_scheme} section-border-top $sectionparentclass" $subsectionparentstyle >
                                <div class="box-header box-header-section  with-border bg-{$this->color_scheme} section-color" $subsectionstyle data-widget="collapse" data-target="#lab" data-mini="true" data-theme="b">
                                    <h4 class="box-title text-right"><span class="title">{$field['label']}</span><span class="headerChange"></span></h4> 
                                    <a class="chevronBtnUp pull-left"   data-widget="collapse"><span class="$chevronclass icon-large" $iconstyle ></span></a>                                    
                                </div>
                                <div id="lab"  class="box-body $sectionchildclass" data-inset="true" data-theme="e" style="text-align:right" data-content-theme="d" >
								{$fielddatamap}
                            </div>
                            </div>
                            </div>
...
//                            if($flow == 2) return;
        ...
        $requiredonflow = $this->getKeyValue($field,'requiredonflow');  
        ...
        if(!empty($requiredonflow)){

            switch($requiredonflow){
                case 1: if($this->viewType == 'create')$label_html .= '<span class="fb-required">*</span>'; break;
                case 2: if($this->viewType == 'edit' || $this->viewType == 'viewandsign ')$label_html .= '<span class="fb-required">*</span>'; break;
                case 3:  $label_html .= '<span class="fb-required">*</span>'; break;
            }
...
<{$field['subtype']} class='form-control span12'>
            {$field_data} 
        </{$field['subtype']}>
FIELDDATA;
                            break;
...
FIELDDATA;
        ...
        $label_name_code = str_replace('phonenumber','phonenumber_code',$label_name);
...
VIEWH;
        ...
        if($this->viewType == 'edit'){
            if($steptoshown == 1){
                $steptoshownstyle = ' style="display:none" ';
        ...
        $element = <<<ELEMENT
    <div class=" box-body form-group" {$hidediv} {$steptoshownstyle} >
        {$field_data}
    </div>
...
//                            $valuex = $this->formGroupViewtags($field, $value);
            ...
            case 'editabletext' :    $valuex = $this->formEditableTextTag($field,array(),$value);
                                     break;
            case 'phonenumber' :    $valuex = $this->formPhonenumberTag($field,array(),$value);
                                    break;
    ...
    public function formSignatureTag($field = array(), $attributes = array(), $value = '') {
        ...
        if(in_array($this->viewType,array('edit','viewandsign', 'create','pdflabeler'))){
            ...
            if(!empty($requiredonflow)){
                ...
                switch($requiredonflow){
                    case 1: if($this->viewType == 'create')$attributes['class'] .= ' requiredfield ';break;
                    case 2: if($this->viewType == 'edit' || $this->viewType == 'viewandsign ')$attributes['class'] .= ' requiredfield '; break;
                    case 3:  $attributes['class'] .= ' requiredfield '; break;
                }
...
//        $attributesbtn['class'] =  ' pull-left file-upload-button ';
        ...
        $label =   stripslashes( $attributesbtn['label']);
    ...
    public function getTag($field = array(),$value = null) {
        ...
        if(is_array($field)){
            ...
            if(!empty($requiredonflow)){
                ...
                switch($requiredonflow){
                    case 1: if($this->viewType == 'create') $className .= ' requiredfield '; break;
                    case 2: if($this->viewType == 'edit' || $this->viewType == 'viewandsign ') $className .= ' requiredfield '; break;
                    case 3:  $className .= ' requiredfield '; break;
                }
            ...
            $copiedfrom = $this->getKeyValue($field,'copiedfrom',false);
            $bgcolor = $this->getKeyValue($field, 'bgcolor',false);
            $textcolor = $this->getKeyValue($field, 'textcolor',false);
            // $errorMessage = $this->getKeyValue($field, 'errorMessage',false);
//            $hr = $this->getKeyValue($field,'hr',false);
...
//                                    'required' => $required,
            ...
            if(isset($copiedfrom) && !empty($copiedfrom)){
                 $attributes['attr-copiedfrom'] = $copiedfrom;
            }
...
//                                        $attributes['readonly'] = 'readonly';
            ...
            switch ($type){
                ...
                case "textarea" :   
                                ...
                                if(!empty($maxlength2) )
                                 ...
                                 if(isset($this->autosave) && $this->autosave == 1){
                                   ...
                                   if(isset($copiedfrom) && !empty($copiedfrom)){
                                    $datesizetext = ' attr-copiedfrom="'.$copiedfrom.'"';
                               }
                ...
                case "address" :   
                                    ...
                                    $attributes['type'] = 'text'; 
                                    $attributes['class'] = 'form-control span6'; 
...
//                                    $tag = html_tag('input', $attributes);
            ...
            case 'phonenumber' :    $tag = $this->formPhonenumberTag($field,array(),$value);break;
                case "hidden"  :    
                                    $attributes['type'] = 'hidden';
        ...
        $tag .= $this->addErrorMessage($field);
     ...
     * 
     */
    public function addErrorMessage($field){

        $errorMessage = $this->getKeyValue($field,'errorMessage', '');
        $tag = '';

        if(!empty($errorMessage)){

            $attributes['class'] = 'span12 validationErrorMsg '; 
            $attributes['style'] = 'display:none;'; 

            $tag = html_tag('p', $attributes,$errorMessage);
        }
    ...
    public function formSelectTag($field = array(),$value = null) {
        ...
        $tag = '<select type="select" class="'.$className.'" '.$href.'  '.$module.' id="'.$fieldid.'" '.$selectvalue.'  name="'.$name.'" '.$disabled.' '.$multiple.' >'.$options.'</select>';
...
TAG;
    ...
    public function formGrouptags($type = '',$field = '',$form = '',$value = null) {
        ...
        $name = $this->getKeyValue($field,'name');  
        $requiredonflow = $this->getKeyValue($field,'requiredonflow',false);  
        $required = $this->getKeyValue($field,'required',false);  
...
//                $temp['name'] = \Lang::get($label)?\Lang::get($label):$label;            
    ...
    public function action_setSettings($module = null) {
        ...
        $settinglist = $this->getSettingFlags();
         ...
         $whatsapp_image = '';   
                ...
                $sysobj = $data->get_one();
                 ...
                 if(!is_object($sysobj)){ 
                     ...
                     $sysobj->name = $module; 
                        ...
                        $datajson = $sysobj->systemsconfig;
                        $postdata = json_decode($datajson, true);
                 }
                 ...
                 foreach ($settinglist as $list){
                    ...
                    //                    if(in_array($list['name'], array('email_subject','email_content'))) continue;
                                    ...
                                    if(empty($postdata['emailreplyto'])){
                                        $postdata['emailreplyto']  =  $postdata['emailTo'];  
                                    }

                                    $postdata['logo']  = \Input::post('logo');  
                                    $postdata['logo_id']  = \Input::post('logo');  
                                    $postdata['backgroundimage_id']  = \Input::post('backgroundimage');  
                                    $postdata['backgroundimage']  = \Input::post('backgroundimage');  
                                    $postdata['whatsapp_image']  = \Input::post('whatsapp_image');  
                    ...
                    $sysobj->enableapi = \Input::post('enableapi');  
                    $sysobj->apikey = \Input::post('apikey'); 
                    $sysobj->callback_url = \Input::post('callback_url');   
                    $sysobj->fields_to_be_sent = \Input::post('fields_to_be_sent');  
                    $sysobj->backgroundimage_id = \Input::post('backgroundimage');  
                    
                    $sysobj->enablePermanantDelete = \Input::post('enablePermanantDelete');  
            ...
            if(is_object($info)  ){
                //Show info
                $inforay['enableapi'] = $info->enableapi;
                $inforay['apikey'] = $info->apikey; 
                $inforay['callback_url'] = $info->callback_url;
                $inforay['fields_to_be_sent'] = $info->fields_to_be_sent;
                $inforay['enablePermanantDelete'] = $info->enablePermanantDelete;
            }
            if(is_object($info) && !empty($info->systemsconfig)){
                //Show info
    ...
    public function getSettingFlags() {
        ...
        if(is_object($userLoggedIn) &&  $userLoggedIn->username  == 'liat'){
...
//                                    array('id'=>2,'type'=>'text','name'=>'pdf_creator','visible'=>1,'group_id'=>1),
                                    ...
                                    array('id'=>34,'type'=>'checkbox', 'value'=> 1,'title'=> 'Enable PDF Preview','name'=>'enable_pdf_preview','visible'=>1,'group_id'=>1),
     ...
     array('id'=>47,'type'=>'checkbox','title'=>'הסר אפשרות לעדכן תאריך תפוגה של הקישור בטופס דו שלבי', 'name'=>'hide_link_expire_date','visible'=>1,'group_id'=>1,  ),  
     array('id'=>29,'type'=>'checkbox', 'title'=>'Logo Portrait Mode', 'name'=>'logo_portrait_mode','visible'=>1,'group_id'=>1, ), 
     array('id'=>12,'type'=>'select','title'=>'One Step Form Download PDF/Show in Browser', 'name'=>'pdf_mode','visible'=>1,'group_id'=>1, 'options' => array('1'=>'Download' , '2' => 'Show in Browser', '3' => 'Show in Browser  & Download', '4' => '-- None --')), 
   array('id'=>30,'type'=>'select','title'=>'Two Step Form Download PDF/Show in Browser', 'name'=>'pdf_mode_flow2','visible'=>1,'group_id'=>1, 'options' => array('1'=>'Download' , '2' => 'Show in Browser', '3' => 'Show in Browser  & Download', '4' => '-- None --')), 
    ...
    array('id'=>45,'type'=>'checkbox','title'=>'Enable SMS', 'name'=>'enablesms','visible'=>1,'group_id'=>1,  ), 
    array('id'=>46,'type'=>'textbox','title'=>'Whatsapp Customer Name', 'name'=>'custname','visible'=>1,'group_id'=>1,  ),  
    array('id'=>46,'type'=>'textbox','title'=>'SMS Sender name', 'name'=>'sms_sender_name','visible'=>1,'group_id'=>1,  ),  
    array('id'=>47,'type'=>'checkbox','title'=>'מחיקת רשומה לאחר שליחה למייל (מתאים גם לדו שלבי) ', 'name'=>'dont_store_data','visible'=>1,'group_id'=>1,  'disabled' => $disabled   ),  
...
//                                    array('id'=>32,'type'=>'textbox','name'=>'watermarktext','title'=>'Watermark Text','visible'=>1,'group_id'=>1), 
//                                    array('id'=>33,'type'=>'textbox','name'=>'watermarktextcolor','title'=>'Watermark Text Color','visible'=>1,'group_id'=>1), 
                                    array('id'=>5,'type'=>'textbox','name'=>'pdf_template_name','title'=>'PDF Template Name ','visible'=>1,'group_id'=>1), 
                                    array('id'=>33,'type'=>'textbox','name'=>'pdf_report_name','title'=>'PDF Report Name','visible'=>1,'group_id'=>1), 
                                    ...
                                    array('id'=>49,'type'=>'textbox','name'=>'emailreplyto','title'=>'Email Reply To','visible'=>1,'group_id'=>1), 
                                    array('id'=>8,'type'=>'textbox','name'=>'senderName','title'=>'Email Sender Name','visible'=>1,'group_id'=>1), 
                                    array('id'=>51,'type'=>'checkbox', 'title'=>'Enable API', 'name'=>'enableapi','visible'=>1,'group_id'=>1, ), 
                                    array('id'=>52,'type'=>'textbox', 'title'=>'API key', 'name'=>'apikey','visible'=>1,'group_id'=>1, 'attributes' => array('readonly' => 'readonly') ), 
                                  array('id'=>14,'type'=>'textbox','name'=>'currentsequence','title'=>'Form Sequence','visible'=>1,'group_id'=>1), 
                                    array('id'=>39,'type'=>'checkbox', 'value'=> 1,'title'=>'Auto Numbering','name'=>'auto_numbering','visible'=>1,'group_id'=>1), 
                                    array('id'=>49,'type'=>'checkbox', 'value'=> 1,'title'=>'Enable Export CSV','name'=>'enableExportCSV','visible'=>1,'group_id'=>1), 
                                    array('id'=>49,'type'=>'textbox', 'value'=> 1,'title'=>'Form Title','name'=>'formIndexTitle','visible'=>1,'group_id'=>1), 
...
//                                    array('id'=>11,'type'=>'select', 'name'=>'flowType','title'=>'','visible'=>1,'group_id'=>1, 'options' => array('1'=>'1' , '2' => '2')), 
                                   //                                    array('id'=>13,'type'=>'file','title'=>'העלה לוגו טופס', 'name'=>'logo','visible'=>1,'group_id'=>1, ), 
                                    ...
                                    array('id'=>34,'type'=>'textbox','title'=>'Add Border color', 'name'=>'bordercolor','visible'=>1,'group_id'=>1, ), 
                                    array('id'=>35,'type'=>'textbox','title'=>'Add Icon color', 'name'=>'iconcolor','visible'=>1,'group_id'=>1, ), 
...
array('id'=>9,'type'=>'checkbox', 'value'=> 1,'title'=>' Enable Delete Permanent Data ','name'=>'enablePermanantDelete','visible'=>1,'group_id'=> 1), 
...
array('id'=>18,'type'=>'checkbox', 'value'=> 1,'title'=>'Add Accessibility Menu', 'name'=>'addAccessibilityMenu','visible'=>1,'group_id'=>1 ), 
//                                    array('id'=>15,'type'=>'textbox','title'=>'whatsapp image', 'name'=>'whatsapp_image','visible'=>0,'group_id'=>1, ), 
      ...
      array('id'=>50,'type'=>'text', 'elementid'=>'system_type','title'=>'systemType', 'name'=>'system_type','visible'=>0,'group_id'=>1, ), 

      array('id'=>60,'type'=>'checkbox',  'title'=>'Set Create outside Login', 'name'=>'create','visible'=>1,'group_id'=>3, ), 
      array('id'=>61,'type'=>'checkbox',  'title'=>'Set Edit outside Login', 'name'=>'edit','visible'=>1,'group_id'=>3, ), 
      array('id'=>62,'type'=>'checkbox',  'title'=>'Set Viewandsign outside Login', 'name'=>'viewandsign','visible'=>1,'group_id'=>3, ), 
      array('id'=>63,'type'=>'checkbox',  'title'=>'Set View outside Login', 'name'=>'view','visible'=>1,'group_id'=>3, ), 
      array('id'=>63,'type'=>'checkbox',  'title'=>' ', 'name'=>' ','visible'=>1,'group_id'=>5, ), 
...
//                                        $field['name'] = $original_name;*/
        ...
        $attributes['value'] = str_replace('bankdetails', 'branchcode', $value);  
            ...
            $attributes['class'] = ' form-fields popup-autocomplete popup-autocomplete-product-key'
                    . ' form-control span6 ui-autocomplete-input ';
            if($this->hasManyData ==  1){
...
//                                            print_r($attributes['href2']); die();
        ...
        $attributes['value'] = str_replace('bankdetails', 'branch_id', $value);  
            $attributes['type'] ="hidden";
            $attributes['map-controller'] ="$mod/$con"; 
            $attributes['class'] = 'form-fields updateflag  form-control span6';
    ...
    public function formEditableTextTag($field, $attributes, $value) {
        ...
        $checkboxmarked = $this->getKeyValue($field,'checkboxmarked',false); 
        $savelatest = $this->getKeyValue($field,'savelatest',false); 
//        echo $checkboxrequired; die();
        ...
        $savebuttonname =  str_replace("editabletext","button",$name);
        ...
        if($this->hasManyData ==  1){
            ...
            $savebuttonid =  str_replace("editabletext","button",$ckoriid);
        ...
        if($this->viewType == 'edit' || $this->viewType == 'view'){
            ...
            if($this->viewType == 'create' ||  $this->viewType == 'pdflabeler'){
                ...
                if(isset($checkboxmarked) && $checkboxmarked == 'yes'){
                    $ckattributes['checked'] = 'checked';
                }
                ...
                $savebuttonattributes =  array( 'class' => ' btn btn-primary pull-left saveLatestData hideText',           
                                                'name' => $savebuttonname,
                                                'id' => $savebuttonid ,
                                                'text-id' => $textname, 
                                                );
                 if($this->hasManyData ==  1){
                    ...
                    $ckattributes['model'] = $model;
                     ...
                     $savebuttonattributes['text-id'] = $model.'-'.$textid.'-count';
//                      $textidmap =  str_replace("editabletext","atag",$ckoriid);
                ...
                $latestdata = '<?php if(isset($'.$textname.') && $'.$textname.' == 1 ){ ?>';
                $latestdata .= '<?php if(isset($latestorder) && is_object($latestorder) ){ ?>';
                $latestdata .= '<?php echo $latestorder->'.$textname.' ?>';
                $latestdata .= '<?php }else{ ?>';
                $latestdata .= $label;
                $latestdata .= '<?php } }else{ ?>';
                $latestdata .= $label;
                ...
                $texttag = html_tag('textarea', $textattributes, $latestdata);
                $savebuttontag = html_tag('button', $savebuttonattributes, 'שמור שינויים שביצעתי בטקסט');
                if( $this->viewType == 'pdflabeler'){
                    $texttag .= "{$textname}";
                }
...
//                $divtag = html_tag('div', $divattributes, htmlspecialchars_decode($label));
                $divtag = html_tag('div', $divattributes, htmlspecialchars_decode($latestdata));

                $divtag .= $texttag;

                if($savelatest == 'yes')
                $divtag .= $savebuttontag;
    ...
    public function formPhonenumberTag($field, $attributes, $value) {

        $required = $this->getKeyValue($field,'required',false); 
        $requiredonflow = $this->getKeyValue($field,'requiredonflow',false); 
        $className = $this->getKeyValue($field,'className',false); 
        $name = $this->getKeyValue($field,'name');    
        $disabled = $this->getKeyValue($field,'disabled',false);  
        $fieldid = $this->getKeyValue($field,'name',false); 
        
        $className =  trim($className,"span6");
   
        $className =  trim($className,"span6");

        $className  .= ' form-control ';
        
        $phonetype = $this->getKeyValue($field,'phonetype',false);
            
        $phonecode = str_replace('phonenumber', 'phonenumber_code', $name);
        
        $phonecodeid = str_replace('phonenumber', 'phonenumber_code', $name);

        if($phonetype == '1')
        $options =array('', '050', '051', '052', '053', '054', '055','056','058','059');
        else
        $options =array('', '02', '03', '04', '08', '09', '072','073','074','076', '077', '079');

        $opt = '';
        $optview = '';
        if($this->hasManyData == 1){ 
            $mapvariable = 'line';
            $mapvariable = '$'.$mapvariable.'->{"'.$phonecodeid.'"}';
        }else{
            $mapvariable = 'order';
            $mapvariable = '$'.$mapvariable.'->{"'.$phonecodeid.'"}';
        }
        ...
        foreach($options as $ops ){
            ...
            $selectvalue = ' <?php if(isset('.$mapvariable.') && '.$mapvariable.' == "'.$ops.'" ) { echo "selected";   } ?> ';
            $opt .= html_tag('option', array($selectvalue,'value'=>$ops),$ops);

            $optview .= '<?php if(isset('.$mapvariable.') && '.$mapvariable.' == "'.$ops.'" ) { echo '.$mapvariable.';   } ?> ';
        }
//   echo $opt; die();
        $tags= '';
            

        $divattr = array( 'name'=> $name);

       
        $className2 = $className.' span2 phoneNumberCodeBox';
        
        // if($required == 1) $className2 .= ' requiredfield ';
        
        if(!empty($requiredonflow)){

            switch($requiredonflow){
                case 1: if($this->viewType == 'create') $className2 .= ' requiredfield '; break;
                case 2: if($this->viewType == 'edit' || $this->viewType == 'viewandsign ') $className2 .= ' requiredfield '; break;
                case 3:  $className2 .= ' requiredfield '; break;
            }
            
        }

        $attributes = array(   'type' => 'phone_number',
        'class' => $className.' span4 phoneNumberBox',
        // 'label' => $label, 
        'placeholder' => '',
        'value' => $value, 
        'autocomplete' => 'off', 
    ...
    $attributes['value'] = '<?php if(isset('.$mapvariable.')){ echo '.$mapvariable.'; } ?>' ;
    if(in_array($this->viewType, array('create','edit','pdflabeler')) ){
        ...
        $attributes['attr-name'] = $phonecode;
        ...
        $attributes['value'] = '';
        $tag .='&nbsp;<b>-</b> &nbsp;';
        $tag .= '<select class="'.$className2.'"   id="'.$fieldid.'"  name="'.$phonecode.'"   >'.$opt.'</select>';
    ...
    public function getBankOptions($field, $field, $value, $id) {
        ...
        $attributes['class'] = ' form-fields  form-control span6 ';
        $attributes2['class'] = ' form-fields  form-control span6 ';
