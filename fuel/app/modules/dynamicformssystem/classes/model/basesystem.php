...
//           
    ...
    public static function preInit($table) {
        ...
        try{
            ...
            \DBUtil::create_table($table, static::$_properties,array("id"), true, 'MyISAM');
...
//        
    ...
    public function GetFileProperties(){
        ...
        foreach($arr as $property=>$val){
                 if(isset($val['form']) and isset($val['form']['type']) and  $val['form']['type'] === 'file')
                   $temp[] = $property;
        }
        return $temp;
    }
    ...
    public function GetSendLinkSMSProperty(){
             
            $temp = '';
            $arr = $this->properties();
            foreach($arr as $property=>$val){
                     if(isset($val['send_link_sms']) and  $val['send_link_sms'] === '1' && $val['fieldtype'] == 'phonenumber' )
                       $temp = $property;
            }
            return $temp;
    }

    public function GetSendLinkWhatsappProperty(){
             
        $temp = '';
        $arr = $this->properties();
        foreach($arr as $property=>$val){
                 if(isset($val['send_link_whatsapp']) and  $val['send_link_whatsapp'] === '1' && $val['fieldtype'] == 'phonenumber' )
                 $temp = $property;
        }
        return $temp;
    }
    
    public function GetSendLinkSMSCodeProperty(){
             
        $temp = '';
        $arr = $this->properties();
        foreach($arr as $property=>$val){
                 if(isset($val['send_link_sms']) and  $val['send_link_sms'] === '1' && $val['fieldtype'] == 'phonecode' )
                   $temp = $property;
        }
        return $temp;
    }

    public function GetSendLinkWhatsappCodeProperty(){
            
        $temp = '';
        $arr = $this->properties();
        foreach($arr as $property=>$val){
                if(isset($val['send_link_whatsapp']) and  $val['send_link_whatsapp'] === '1' && $val['fieldtype'] == 'phonecode' )
                $temp = $property;
        }
        return $temp;
    }

    public function GetSendLinkEmailProperty(){
            
        $temp = '';
        $arr = $this->properties();
        foreach($arr as $property=>$val){
                if(isset($val['send_link_email']) and  $val['send_link_email'] === '1')
                $temp = $property;
        }
        return $temp;
    }

    public function getSaveLatestParams(){
             
                $temp = array();
                $arr = $this->properties();
                foreach($arr as $property=>$val){
                         if(isset($val['savelatest']) and  $val['savelatest'] === '1')
                           $temp[] = $property;
                }
                return $temp;
        }
